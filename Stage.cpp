...
Stage::~Stage()
{
	...
	/*Destroy(bill), */ Destroy(camera);
	Destroy(entities);
	Destroy(backgroundTerrains), Destroy(foregroundTerrains);
}
...
void Stage::Render()
{
	...
	for (auto &[backgroundTerrain, node] : backgroundTerrainsResult)
...
void Stage::LoadBackgroundTerrains(void *backgroundTerrainsLayer)
{
	...
	for (auto &[tileObjectPosition, tileObject] : _backgroundTerrainsLayer->getTileObjects())
	{
		...
		auto animationId = std::to_string(tileObject.getTile()->getId());
		auto &position = tileObject.getPosition();
		auto &size = tileObject.getTile()->getTileSize();
	...
	_backgroundTerrains.sort([](Entity *e1, Entity *e2) -> bool
