...
{
	if (input.IsKey(DIK_UP  ) && selection == SELECTION::END     ) selection = SELECTION::CONTINUE, atRow = AtRow(10.5f);
	else
	if (input.IsKey(DIK_DOWN) && selection == SELECTION::CONTINUE) selection = SELECTION::END     , atRow = AtRow(08.5f);

	if (input.IsKey(DIK_RETURN))
	{
		switch (selection)
		{
		case SELECTION::CONTINUE: { --scene.currentStage    ; scene.currentScore = 0; return new LoadingSceneState(); }
		case SELECTION::END     : {   scene.currentStage = 0; scene.currentScore = 0; return new   StartSceneState(); }
		}
	}
