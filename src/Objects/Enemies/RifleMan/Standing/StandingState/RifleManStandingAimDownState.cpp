...
void RifleManStandingAimDownState::Enter(RifleManStanding& rifleManStanding)
{
	if (this->UpdateShooting(rifleManStanding))
	{
		...
		{
			rifleManStanding.CustomFire(position.x - w * 0.5f, position.y + h * Constants::Enemies::RifleMan::AIM_DOWN_GUN_OFFSET_RATIO_Y, 0.0f, vx, vy, 0.0f, 0.0f, movingDirection);
		}
		else
		{
			rifleManStanding.CustomFire(position.x + w * 0.5f, position.y + h * Constants::Enemies::RifleMan::AIM_DOWN_GUN_OFFSET_RATIO_Y, 0.0f, vx, vy, 0.0f, 0.0f, movingDirection);
