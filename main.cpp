...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	while (TRUE)
	{
		...
		pio.t = poo.t;
		GraphicsHelper::DrawSprite(GraphicsDatabase::sprites[SOLDIER_SPRITE_ID::RUN_01], D3DXVECTOR3(x, poo.c, 0.0f), DIRECTION::RIGHT, 45.0f);
		x += vx;
		if (x >= 500.0f || x <= 0.0f)
			vx = -vx;
		...
		GraphicsHelper::DrawSprite(GraphicsDatabase::sprites[BILL_SPRITE_ID::RUN_01], D3DXVECTOR3(pip.x, pip.y, 0.0f), DIRECTION::RIGHT, -45.0f);

		auto poc = Motion::CalculateUniformCircularMotion(pic);
		pic.ω = poc.ω;
		GraphicsHelper::DrawSprite(GraphicsDatabase::sprites[BILL_SPRITE_ID::RUN_02], D3DXVECTOR3(poc.x, poc.y, 0.0f), DIRECTION::RIGHT, angle);
		angle += dAngle;
		//
