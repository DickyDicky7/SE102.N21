...
void QuadTreeNode::Clean()
{
	for (int i = 0; i <= 3; i++)
		if (this->nodes[i])
			this->nodes[i]->Clean(), Destroy(this->nodes[i]);
...
void QuadTreeNode::Insert(Entity *entity)
{
	if (this->Contain(entity))
	{
		...
		{
			if (!this->nodes[0])
				this->nodes[0] = new QuadTreeNode{this->x, this->y, this->w * 0.5f, this->h * 0.5f};
			if (!this->nodes[1])
				this->nodes[1] = new QuadTreeNode{this->x + this->w * 0.5f, this->y, this->w * 0.5f, this->h * 0.5f};
			if (!this->nodes[2])
				this->nodes[2] = new QuadTreeNode{this->x + this->w * 0.5f, this->y + this->h * 0.5f, this->w * 0.5f, this->h * 0.5f};
			if (!this->nodes[3])
				this->nodes[3] = new QuadTreeNode{this->x, this->y + this->h * 0.5f, this->w * 0.5f, this->h * 0.5f};
...
void QuadTreeNode::Remove(Entity *entity) // THIS METHOD IS STILL UNDER EXPERIMENTS
{
	if (this->Contain(entity))
	{
		if (this->entities.remove(entity) > 0)
			return;
		for (int i = 0; i <= 3; i++)
		{
...
{
	return entity->GetB() >= this->GetB() && entity->GetT() <= this->GetT() && entity->GetL() >= this->GetL() && entity->GetR() <= this->GetR();
}

BOOL QuadTreeNode::Contain(Camera *camera)
{
	return camera->GetB() >= this->GetB() && camera->GetT() <= this->GetT() && camera->GetL() >= this->GetL() && camera->GetR() <= this->GetR();
}

BOOL QuadTreeNode::Contain(BoundingBox boundingBox)
{
	return boundingBox.B >= this->GetB() && boundingBox.T <= this->GetT() && boundingBox.L >= this->GetL() && boundingBox.R <= this->GetR();
}

BOOL QuadTreeNode::Intersect(Entity *entity)
{
	return !(entity->GetB() >= this->GetT() || entity->GetT() <= this->GetB() || entity->GetL() >= this->GetR() || entity->GetR() <= this->GetL());
}

BOOL QuadTreeNode::Intersect(Camera *camera)
{
	return !(camera->GetB() >= this->GetT() || camera->GetT() <= this->GetB() || camera->GetL() >= this->GetR() || camera->GetR() <= this->GetL());
}

BOOL QuadTreeNode::Intersect(BoundingBox boundingBox)
{
	return !(boundingBox.B >= this->GetT() || boundingBox.T <= this->GetB() || boundingBox.L >= this->GetR() || boundingBox.R <= this->GetL());
}
...
void QuadTreeNode::Retrieve(BoundingBox boundingBox, std::unordered_map<Entity *, QuadTreeNode *> &result)
{
	...
	else if (this->Intersect(boundingBox))
	{
		if (this->IsSmallestNode())
		{
			for (auto &entity : entities)
...
{
	return this->w == NODE_SMALLEST_W && this->h == NODE_SMALLEST_H;
}
...
{
	return !(entity->GetB() >= this->T || entity->GetT() <= this->B || entity->GetL() >= this->R || entity->GetR() <= this->L);
}
